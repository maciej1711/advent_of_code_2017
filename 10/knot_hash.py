...
def knot_hash(file_name):
    ...
    for length in lengths:
        ...
        for temp_source in range(position, position + length):
            temp.append(range_list[temp_source % len(range_list)])
        for temp_counter in range(length):
            range_list[(position + temp_counter) % len(range_list)] = temp[-temp_counter - 1]
        position = position + length + skipsize
        position = position % 256
        skipsize += 1

    print "Multiplied value of two first items in the know list: %i"%(range_list[0] * range_list[1])
