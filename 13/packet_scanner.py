...
def avoid_scanner(file_name):
    ...
    total_range = int(content[-1][0])
    items = {int(item[0]): int(item[1]) for item in content}
    severity = 0
    counter = 0
    for number in range(total_range+1):
        if number in items:
            if (number) % ((items.get(number) *2)-2) == 0:
...
def main():
    parser = argparse.ArgumentParser(
            description="Script implements algorythm that help bypass the so-called scanner from the task description"
                        "Execute the script by typing python packet_scanner.py --name file_name",
        )
    parser.add_argument('--name', help='Name of a file to open', nargs='*')
    args = parser.parse_args()
    avoid_scanner(args.name)
